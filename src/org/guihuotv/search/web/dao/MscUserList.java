...
/**
 * Title: Base table MscUser 列表类
 * Description: 本文件使用XCodeFactory自动生成。
 * 该类使用Value Object的方式通过List返回数据给前台。
 * 并支持使用ArrayList指定列获取列表，并返回。
 */
public class MscUserList implements DataList<MscUser>{
	
	private static final Logger logger = LoggerFactory.getLogger(MscUserList.class);
	 ...
	 * 返回的value object数组
	 */
	private List<MscUser> results = null;
	/**
	 * 指定字段的list
	 */
	 ...
	 */
	public MscUserList(DAOFactoryImpl dao,String tableName,String[] colList,String addSQL,Object[] paramList,int startIndex,int resultNum,boolean autoCount) throws TransactionException{
		this.dao = dao;
		...
		MscUserDbList(this.addSQL,this.startIndex,this.resultNum);
	}
	 ...
	 * @param resultNum 返回的结果集大小
	 */
	public void MscUserDbList(String addSQL,int startIndex,int resultNum) throws TransactionException
	{
		Connection con = null;
		...
		try {
			...
			if (resultNum>0&&startIndex>=0){
				...
				sql = (String)po[0];
				results = new ArrayList<MscUser>(this.resultNum);
			}else{
				results = new ArrayList<MscUser>();
			}
			...
			throw new TransactionException( "SQLException in MscUserList.java:MscUserList(): " ,sqle );
		}
		finally {
			try {  pstmt.close(); }
	...
	{
		String countSql =
		"SELECT COUNT(id) AS COUNT FROM "+(tableName==null?"msc_user":tableName);
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = dao.getTransactionController().getConnection((tableName==null?"msc_user":tableName),"read");
			...
			logger.error(sqle.getMessage(),sqle);
			throw new TransactionException( "TransactionException in MscUserList.java:countAllSize(): " , sqle );
		}
		finally {
			try {  pstmt.close(); }
			catch (Exception e) { logger.error(e.getMessage(),e); }
	 ...
	 * @param objects
	 */
	public void reset(List<MscUser> objects){
	...
	public Iterator<MscUser> iterator() {
			return this;
	}
	...
	private void loadData(ResultSet rs) throws Exception{
		if (this.colList==null){
			while(rs.next()){ 
				MscUser mscUser= new MscUser(rs.getLong("id"),DmValueUtils.nullToStr(rs.getString("username")),DmValueUtils.nullToStr(rs.getString("passwd")),DmValueUtils.nullToStr(rs.getString("msc_perm")),DmValueUtils.nullToStr(rs.getString("org_perm")),DmValueUtils.nullToStr(rs.getString("org_name")),DmValueUtils.nullToStr(rs.getString("alias_name")),DmValueUtils.nullToStr(rs.getString("telephone")),DmValueUtils.nullToStr(rs.getString("mobile")),DmValueUtils.nullToStr(rs.getString("email")),rs.getTimestamp("last_logon_date"),DmValueUtils.nullToStr(rs.getString("last_logon_ip")),rs.getTimestamp("create_date"),rs.getTimestamp("modify_date"),rs.getInt("status"));
				mscUser.CURRENT_TABLE_NAME = (tableName==null?null:tableName);
				this.results.add(mscUser);
				this.size++;
			}
		}else{
			while(rs.next()){ 
	...
	private String getSql(){
		...
		if (this.colList==null){
			sb.append("SELECT id,username,passwd,msc_perm,org_perm,org_name,alias_name,telephone,mobile,email,last_logon_date,last_logon_ip,create_date,modify_date,status FROM "+(tableName==null?"msc_user":tableName));
			sb.append(" ");
		}else{
			sb.append(" SELECT ");
