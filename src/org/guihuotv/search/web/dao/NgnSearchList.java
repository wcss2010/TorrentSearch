...
/**
 * Title: Base table NgnSearch 列表类
 * Description: 本文件使用XCodeFactory自动生成。
 * 该类使用Value Object的方式通过List返回数据给前台。
 * 并支持使用ArrayList指定列获取列表，并返回。
 */
public class NgnSearchList implements DataList<NgnSearch>{
	
	private static final Logger logger = LoggerFactory.getLogger(NgnSearchList.class);
	 ...
	 * 返回的value object数组
	 */
	private List<NgnSearch> results = null;
	/**
	 * 指定字段的list
	 */
	 ...
	 */
	public NgnSearchList(DAOFactoryImpl dao,String tableName,String[] colList,String addSQL,Object[] paramList,int startIndex,int resultNum,boolean autoCount) throws TransactionException{
		this.dao = dao;
		...
		NgnSearchDbList(this.addSQL,this.startIndex,this.resultNum);
	}
	 ...
	 * @param resultNum 返回的结果集大小
	 */
	public void NgnSearchDbList(String addSQL,int startIndex,int resultNum) throws TransactionException
	{
		Connection con = null;
		...
		try {
			...
			if (resultNum>0&&startIndex>=0){
				...
				sql = (String)po[0];
				results = new ArrayList<NgnSearch>(this.resultNum);
			}else{
				results = new ArrayList<NgnSearch>();
			}
			...
			throw new TransactionException( "SQLException in NgnSearchList.java:NgnSearchList(): " ,sqle );
		}
		finally {
			try {  pstmt.close(); }
	...
	{
		String countSql =
		"SELECT COUNT(id) AS COUNT FROM "+(tableName==null?"ngn_search":tableName);
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = dao.getTransactionController().getConnection((tableName==null?"ngn_search":tableName),"read");
			...
			logger.error(sqle.getMessage(),sqle);
			throw new TransactionException( "TransactionException in NgnSearchList.java:countAllSize(): " , sqle );
		}
		finally {
			try {  pstmt.close(); }
			catch (Exception e) { logger.error(e.getMessage(),e); }
	 ...
	 * @return
	 */
	public NgnSearch get(int index){
	 ...
	 * @param objects
	 */
	public void reset(List<NgnSearch> objects){
	...
	public Iterator<NgnSearch> iterator() {
			return this;
	}
	...
	private void loadData(ResultSet rs) throws Exception{
		if (this.colList==null){
			while(rs.next()){ 
				NgnSearch ngnSearch= new NgnSearch(rs.getLong("id"),rs.getLong("user_id"),DmValueUtils.nullToStr(rs.getString("searchUrl")),rs.getInt("status"));
	...
	private String getSql(){
		...
		if (this.colList==null){
			sb.append("SELECT id,user_id,searchUrl,status FROM "+(tableName==null?"ngn_search":tableName));
			sb.append(" ");
		}else{
			sb.append(" SELECT ");
