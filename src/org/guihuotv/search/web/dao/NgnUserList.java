...
/**
 * Title: Base table NgnUser 列表类
 * Description: 本文件使用XCodeFactory自动生成。
 * 该类使用Value Object的方式通过List返回数据给前台。
 * 并支持使用ArrayList指定列获取列表，并返回。
 */
public class NgnUserList implements DataList<NgnUser>{
	
	private static final Logger logger = LoggerFactory.getLogger(NgnUserList.class);
	 ...
	 * 返回的value object数组
	 */
	private List<NgnUser> results = null;
	/**
	 * 指定字段的list
	 */
	 ...
	 */
	public NgnUserList(DAOFactoryImpl dao,String tableName,String[] colList,String addSQL,Object[] paramList,int startIndex,int resultNum,boolean autoCount) throws TransactionException{
		this.dao = dao;
		...
		NgnUserDbList(this.addSQL,this.startIndex,this.resultNum);
	}
	 ...
	 * @param resultNum 返回的结果集大小
	 */
	public void NgnUserDbList(String addSQL,int startIndex,int resultNum) throws TransactionException
	{
		Connection con = null;
		...
		try {
			...
			if (resultNum>0&&startIndex>=0){
				...
				sql = (String)po[0];
				results = new ArrayList<NgnUser>(this.resultNum);
			}else{
				results = new ArrayList<NgnUser>();
			}
			...
			throw new TransactionException( "SQLException in NgnUserList.java:NgnUserList(): " ,sqle );
		}
		finally {
			try {  pstmt.close(); }
	...
	{
		String countSql =
		"SELECT COUNT(id) AS COUNT FROM "+(tableName==null?"ngn_user":tableName);
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = dao.getTransactionController().getConnection((tableName==null?"ngn_user":tableName),"read");
			...
			logger.error(sqle.getMessage(),sqle);
			throw new TransactionException( "TransactionException in NgnUserList.java:countAllSize(): " , sqle );
		}
		finally {
			try {  pstmt.close(); }
			catch (Exception e) { logger.error(e.getMessage(),e); }
	 ...
	 * @param objects
	 */
	public void reset(List<NgnUser> objects){
	...
	public Iterator<NgnUser> iterator() {
			return this;
	}
	...
	private String getSql(){
		...
		if (this.colList==null){
			sb.append("SELECT id,user_type,username,passwd,mobile,email,alias_name,birthday,age,gender,remark,school,company,occupation,industry,user_point,last_logon_date,create_date,modify_date,status FROM "+(tableName==null?"ngn_user":tableName));
			sb.append(" ");
		}else{
			sb.append(" SELECT ");
