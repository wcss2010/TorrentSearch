...
/**
 * Title: Base table MscPerm 列表类
 * Description: 本文件使用XCodeFactory自动生成。
 * 该类使用Value Object的方式通过List返回数据给前台。
 * 并支持使用ArrayList指定列获取列表，并返回。
 */
public class MscPermList implements DataList<MscPerm>{
	
	private static final Logger logger = LoggerFactory.getLogger(MscPermList.class);
	 ...
	 * 返回的value object数组
	 */
	private List<MscPerm> results = null;
	/**
	 * 指定字段的list
	 */
	 ...
	 */
	public MscPermList(DAOFactoryImpl dao,String tableName,String[] colList,String addSQL,Object[] paramList,int startIndex,int resultNum,boolean autoCount) throws TransactionException{
		this.dao = dao;
		...
		MscPermDbList(this.addSQL,this.startIndex,this.resultNum);
	}
	 ...
	 * @param resultNum 返回的结果集大小
	 */
	public void MscPermDbList(String addSQL,int startIndex,int resultNum) throws TransactionException
	{
		Connection con = null;
		...
		try {
			...
			if (resultNum>0&&startIndex>=0){
				...
				sql = (String)po[0];
				results = new ArrayList<MscPerm>(this.resultNum);
			}else{
				results = new ArrayList<MscPerm>();
			}
			...
			throw new TransactionException( "SQLException in MscPermList.java:MscPermList(): " ,sqle );
		}
		finally {
			try {  pstmt.close(); }
	...
	{
		String countSql =
		"SELECT COUNT(id) AS COUNT FROM "+(tableName==null?"msc_perm":tableName);
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = dao.getTransactionController().getConnection((tableName==null?"msc_perm":tableName),"read");
			...
			logger.error(sqle.getMessage(),sqle);
			throw new TransactionException( "TransactionException in MscPermList.java:countAllSize(): " , sqle );
		}
		finally {
			try {  pstmt.close(); }
			catch (Exception e) { logger.error(e.getMessage(),e); }
	 ...
	 * @param objects
	 */
	public void reset(List<MscPerm> objects){
	...
	public Iterator<MscPerm> iterator() {
			return this;
	}
	...
	private String getSql(){
		...
		if (this.colList==null){
			sb.append("SELECT id,parent_id,perm_type,perm_name,perm_desc,child_num,action_url,menu_layer,menu_path,create_date,modify_date,status FROM "+(tableName==null?"msc_perm":tableName));
			sb.append(" ");
		}else{
			sb.append(" SELECT ");
