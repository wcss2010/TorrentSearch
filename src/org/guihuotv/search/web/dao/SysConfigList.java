...
/**
 * Title: Base table SysConfig 列表类
 * Description: 本文件使用XCodeFactory自动生成。
 * 该类使用Value Object的方式通过List返回数据给前台。
 * 并支持使用ArrayList指定列获取列表，并返回。
 */
public class SysConfigList implements DataList<SysConfig>{
	
	private static final Logger logger = LoggerFactory.getLogger(SysConfigList.class);
	 ...
	 * 返回的value object数组
	 */
	private List<SysConfig> results = null;
	/**
	 * 指定字段的list
	 */
	 ...
	 */
	public SysConfigList(DAOFactoryImpl dao,String tableName,String[] colList,String addSQL,Object[] paramList,int startIndex,int resultNum,boolean autoCount) throws TransactionException{
		this.dao = dao;
		...
		SysConfigDbList(this.addSQL,this.startIndex,this.resultNum);
	}
	 ...
	 * @param resultNum 返回的结果集大小
	 */
	public void SysConfigDbList(String addSQL,int startIndex,int resultNum) throws TransactionException
	{
		Connection con = null;
		...
		try {
			...
			if (resultNum>0&&startIndex>=0){
				...
				sql = (String)po[0];
				results = new ArrayList<SysConfig>(this.resultNum);
			}else{
				results = new ArrayList<SysConfig>();
			}
			...
			throw new TransactionException( "SQLException in SysConfigList.java:SysConfigList(): " ,sqle );
		}
		finally {
			try {  pstmt.close(); }
	...
	{
		String countSql =
		"SELECT COUNT(id) AS COUNT FROM "+(tableName==null?"sys_config":tableName);
		Connection con = null;
		PreparedStatement pstmt = null;
		try {
			con = dao.getTransactionController().getConnection((tableName==null?"sys_config":tableName),"read");
			...
			logger.error(sqle.getMessage(),sqle);
			throw new TransactionException( "TransactionException in SysConfigList.java:countAllSize(): " , sqle );
		}
		finally {
			try {  pstmt.close(); }
			catch (Exception e) { logger.error(e.getMessage(),e); }
	 ...
	 * @return
	 */
	public SysConfig get(int index){
	 ...
	 * @param objects
	 */
	public void reset(List<SysConfig> objects){
	...
	private void loadData(ResultSet rs) throws Exception{
		if (this.colList==null){
			while(rs.next()){ 
				SysConfig sysConfig= new SysConfig(rs.getLong("id"),DmValueUtils.nullToStr(rs.getString("cat_name")),DmValueUtils.nullToStr(rs.getString("prop_name")),DmValueUtils.nullToStr(rs.getString("prop_value")),DmValueUtils.nullToStr(rs.getString("prop_comment")),DmValueUtils.nullToStr(rs.getString("input_comment")),DmValueUtils.nullToStr(rs.getString("display_order")),rs.getTimestamp("create_date"),rs.getTimestamp("modify_date"),rs.getInt("status"));
				sysConfig.CURRENT_TABLE_NAME = (tableName==null?null:tableName);
				this.results.add(sysConfig);
				this.size++;
			}
		}else{
			while(rs.next()){ 
	...
	private String getSql(){
		...
		if (this.colList==null){
			sb.append("SELECT id,cat_name,prop_name,prop_value,prop_comment,input_comment,display_order,create_date,modify_date,status FROM "+(tableName==null?"sys_config":tableName));
			sb.append(" ");
		}else{
			sb.append(" SELECT ");
